...
def hotpot_to_dict(hotpot_dir):
    """convert the hotpot dataset to dictionary by _id and create paragraph name: paragraph text dictionary within.

    @params hotpot_dir (str): filepath for hotpot dataset.
    """

    hotpot = json.load(open(hotpot_dir))
    hotpot_dict = {}
    ...
    for qa in hotpot:
        ...
        qa_dict['supporting_facts'] = qa['supporting_facts']

        context_dict = {}
        for context in qa['context']:
            context_dict[context[0]] = context[1]

        qa_dict['context'] = context_dict

        qa_id = qa['_id']
        qa_dict['_id'] = qa_id

        hotpot_dict[qa_id] = qa_dict
...
def pred_2_squad(hotpot_dict, preds_dict):
    ...
    for ids in preds_dict.keys():
        ...
        context_list = []
        for para_name, sentence_idx in preds_dict[ids]:
            try:
                context_list.append(entry['context'][para_name][sentence_idx])
            except:
                print(ids,para_name, setnence_idx)
                continue
        ...
        # create dictionary for data entry
        data = {'title' :entry['_id'],
