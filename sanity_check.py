...
def check_compile(config):
    word_mat = np.zeros((395261, 300))
    char_mat = np.zeros((6964, 8))
    ...
    for data in build_train_iterator():
        ...
        print(type(predict_support), predict_support.shape)
        # logit1, logit2, predict_type, predict_support = model(context_idxs, ques_idxs, context_char_idxs,
        #                                                       ques_char_idxs, context_lens, start_mapping,
        #                                                       end_mapping, all_mapping, return_yp=False)
        # # logit1
        # # torch.Size([1, 643])
        ...
        # # predict_support
        # # torch.Size([1, 35, 2])
        # print("logit1", logit1.shape) # start idx?
        # print("logit2", logit2.shape) # end idx?
        # print("predict_type", predict_type.shape) # span/yes/no?
        # print("predict_support", predict_support.shape) # for each sentence, prob of true/false
        break
